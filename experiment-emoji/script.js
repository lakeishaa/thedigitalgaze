...
function openMultipleWindows(url, count) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        let newWindow = window.open(url, "newWindow" + i, "location=no,toolbar=no,menubar=no,scrollbars=yes,resizable=yes,width=" + winW + ",height=" + winH + ",top=" + top + ",left=" + left);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    links.forEach(function(link) {
        link.addEventListener('click', function(event) {
            ...
            openMultipleWindows(this.href, 12); // Pass the href of the link and the number of windows to open
        });
    });
});
